...
def _now_iso() -> str:
    return datetime.now(ZoneInfo("Europe/Prague")).strftime("%Y-%m-%d %H:%M:%S")
...
def delete_team(team_id: int) -> bool:
    doc = _load_index()
    before = len(doc.get("teams", []))
    doc["teams"] = [t for t in doc.get("teams", []) if t["id"] != team_id]
    if len(doc["teams"]) != before:
        _save_index(doc)
        return True
    return False
